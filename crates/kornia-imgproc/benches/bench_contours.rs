...
/// Hollow square (ring): outer margin = size/8, inner margin = size/4.
...
/// Sparse random noise via a simple LCG — no extra dependency needed.
...
/// Register all six variants (kornia free fn ×2, executor ×2, opencv ×2)
/// for a pre-built image pair.
fn register_variants(
    ...
    // -- kornia free function (allocates scratch buffer each call) -----------
    ...
    // -- kornia executor (reuses scratch buffer across calls) ----------------
    // The executor is created outside the iter loop so the buffer is reused,
    // matching real production usage and isolating algorithmic cost.
